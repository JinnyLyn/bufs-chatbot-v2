...
- **`_rejudge.py`** — `_ragas_kpi` 결과 json의 **저장된 생성물로 judge만 교체 재채점**(재생성 X, 100문항×5지표 ≈ 20분). judge 선택이 검색↔생성 귀인을 뒤집는 것이 실측됨(#86: gemma3:4b는 context_recall을 0.9 상수로 채점 → 오답 전부 생성-side 오판; gemma4:26b로 재판정 시 검색-side 61로 역전) — **정규 judge는 gemma4:26b + `--no-think`**, 생성기와 다른 계열 필수(동일하면 abort). 출력 스키마가 `ragas_kpi_*`와 동일해 `_error_buckets`/`_qa_report`가 그대로 소비. `--in <ragas_kpi json> --judge-model … --n N`(스모크는 `--tag` 분리 — 같은 tag면 `_latest`가 축소본으로 덮임).
...
- **`_rank_probe.py`** — **4버킷 검색 rank probe**(#104 방법): 원문 질의를 에이전트 우회로 임베디드 Qdrant hybrid top-k에 직접 조회해 captured@5 / rank_cut / chunking / embedding으로 분류(any-token 기준, threshold 미적용 + sub-threshold 플래그). `--run <ragas_kpi json>` 교차 시 라이브 오답 중 "원문이면 top-5"인 **agent-side loss(term-drift, #87)** 를 분리하고, `--legs`로 dense/sparse 레그 귀속까지 산출(2026-07-08 실측: drift 19건 중 16건 dense-소관 → sparse-만 원문 주입하는 split-path로는 회복 불가, ON A/B 순중립). 오답 컷은 `_error_buckets`와 동일(버킷 라벨 우선, 폴백 `THRESH=0.7`); sub-threshold 히트는 term-drift가 아닌 **threshold-cut**으로 별도 집계. **⚠ 임베디드 Qdrant 단일 프로세스 락 — 백엔드 내리고 실행**, 100문항 ≈ 0.2분. `logs/rank_probe_latest.json` 출력.
