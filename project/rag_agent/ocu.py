...
1. **OCU-topic chunks** (the OCU 안내 sections + 부록 headers): OCU is what the
   ...
   "한국열린사이버대학교", "cons.ocu.ac.kr" … These are the contaminators.
...
Known gap, deliberate: the 부록 course-list *table rows* carry no "OCU" token and
stay unscoped — they only ever rank for course-name queries, where they are the
right answer.

Question side: demotion applies only when the question does NOT mention OCU
(OCU / 오씨유 / 열린사이버 / 공동활용 …). Both detectors fail open: a missed OCU
...
# trigram ("document", "focus" …) never make a question look OCU-scoped.
